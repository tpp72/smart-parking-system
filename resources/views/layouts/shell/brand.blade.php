...
<a href="{{ $nav['homeHref'] ?? url('/') }}"
    ...
    @if ($compact) aria-label="Smart Parking — หน้าหลัก" @endif>
    ...
    @unless ($compact)
        <span @class(['text-body font-semibold leading-tight text-fg', 'sr-only sm:not-sr-only' => $nameFromSm])>Smart Parking</span>
    @endunless
</a>
